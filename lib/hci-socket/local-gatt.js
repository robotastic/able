...
LocalGatt.prototype.onAclStreamData = function(cid, request) {
  if (cid !== ATT_CID) {
    debug("CID != ATT_CID, not looking at it");
    return;
  }
  ...
  if ('2803' === uuid) {
    ...
    if (characteristics.length) {
      ...
      for (i = 0; i < numCharacteristics; i++) {
        ...
        debug('\t\tStart handle: \t' + printBuf.writeUInt16LE(characteristic.startHandle,0).toString('hex') );
        debug('\t\tProperties: \t' + printBuf.writeUInt16LE(characteristic.properties,0).toString('hex') );
        debug('\t\tValue Handle: \t' + printBuf.writeUInt16LE(characteristic.valueHandle,0).toString('hex') );
    ...
    if (secure && !this._aclStream.encrypted) {
      ...
      printBuf.writeUInt16LE(valueHandle,0);
      debug('\t\tValue handle: \t' + printBuf.toString('hex') );
