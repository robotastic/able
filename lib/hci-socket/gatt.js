...
Gatt.prototype.onAclStreamData = function(cid, data) {
  ...
  debug('handing request: ' + request.toString('hex') + " requestType: " + requestType);
  switch(requestType) {
    case ATT_OP_ERROR:
      debug('ATT_OP_ERROR: Request Op Code in error: ' + request[1].toString('hex') + ' Handle in Error: ' + request.readUInt16LE(3).toString('hex') + " Error code: " + request[6].toString('hex'));
      break;
    case ATT_OP_READ_BY_TYPE_RESP:
    case ATT_OP_READ_BY_GROUP_RESP:
      debug('ATT_OP_READ_BY_TYPE_RESP: ');
      break;
    case ATT_OP_MTU_REQ:
    ...
    case ATT_OP_MTU_RESP:
      debug("ATT_OP_MTU_RESP");
      response = this.handleMtuResponse(request);
    break;
  ...
  if (this._currentCommand && data.toString('hex') === this._currentCommand.buffer.toString('hex')) {
    ...
    debug(this._address + ': ignoring request/command: ' + requestType);
...
Gatt.prototype.findByTypeRequest = function(startHandle, endHandle, uuid, value) {
  ...
  for (var i = 0; i < value.length; i++) {
    buf.writeUInt8(value.readUInt8(i), i + 7);
  }
...
Gatt.prototype.handleMtuRequest = function(request) {
  ...
  this.emit('handleMtuRequest', this._mtu);
...
Gatt.prototype.handleMtuResponse = function(data) {
    var opcode = data[0];

    if (opcode === ATT_OP_MTU_RESP) {
      var newMtu = data.readUInt16LE(1);
        debug("MTU Reqsponse: " + newMtu);
