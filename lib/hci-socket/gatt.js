...
Gatt.prototype.readByGroupRequest = function(startHandle, endHandle, groupUuid) {
  ...
  debug('Sending ATT_OP_READ_BY_GROUP_REQ - startHandle: 0x' + startHandle.toString(16) + ' endHandle: 0x' + endHandle.toString(16) + ' Group UUID: 0x' + groupUuid.toString(16));
...
Gatt.prototype.findByTypeRequest = function(startHandle, endHandle, uuid, value) {
  ...
  var callback = function(data) {
    ...
    if (opcode === ATT_OP_FIND_BY_TYPE_RESP) {
      ...
      debug('\tATT_OP_FIND_BY_TYPE_RESP Found Service UUID: ' + uuid + " start- " + start + " end - " + end);
      this.emit('servicesDiscover', this._address, uuid);
